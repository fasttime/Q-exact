...
(function (self)
{
    ...
    function addFactors(op1, op2, operationSign)
    {
        ...
        if (sign1 && sign2)
        {
            ...
            var numerator = getCommonNumerator(op1, op2, operationSign, factors);
        ...
        else
        {
            ...
            factors = op1.factors || op2.factors;
        }
    ...
    function getCommonNumerator(op1, op2, operationSign, factors)
    {
        var factor;
        ...
        var oddProduct1 = op1.sign;
        var oddProduct2 = op2.sign;
        ...
        var product2 = operationSign * oddProduct2 * binPow(binDiff2);
        var result = product1 + product2;
        if (result - product1 !== product2 || result - product2 !== product1)
        {
            throwArithmeticOverflow();
        }
        return result;
    ...
    var qCompareTo =
        function (op)
        {
            ...
            {
                var numerator = getCommonNumerator(this, op, -1, createFactors());
                result = numerator / Math.abs(numerator) ^ 0;
            }
            return result;
        };
    ...
    /**
     * @function Q.compare
     * @param {Q} comparand1 First rational to be compared.
     * @param {Q} comparand2 Second rational to be compared.
     * @returns
     * {number} 1 if the first rational is large than the second one, -1 if the first rational is
     ...
     * @param {Q} comparand Rational to compare with `this`.
     * @returns
     * {number} 1 if `this` is larger than the comparand, -1 if it is smaller, or 0 if they are
     * equal.
     */
